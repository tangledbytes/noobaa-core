...
// When custom CA certs are found, we must also include the default root certificates
// because setting the `ca` option on an https.Agent completely replaces (not extends)
// the default Mozilla/system CA list. Without this, connections to publicly-trusted
// endpoints (AWS, Azure, GCP, etc.) would fail when only partial custom CAs are loaded.
